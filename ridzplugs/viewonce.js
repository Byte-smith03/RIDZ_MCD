...
let handler = async (m, { trashcore, trashown, reply }) => {
  ...
  try {
    ...
    if (isViewOnceImage) {
      ...
      await trashcore.sendMessage(m.chat, { image: buffer, caption: `*Retrieved by Ridz MCD*\n\n*ViewOnce:* Image\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
      return;
    }
    ...
    if (isViewOnceVideo) {
      ...
      const caption = mediaMessage.caption || '';
      await trashcore.sendMessage(m.chat, { video: fs.readFileSync(tempFile), caption: `*Retrieved by Ridz MCD*\n\n*ViewOnce* Video\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
